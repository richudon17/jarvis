# JARVIS — Codebase Overview (Phase 1) — Updated Master Notes

## Summary
JARVIS is a Python CLI autonomous agent that turns a user **goal** into an LLM-generated sequence of **tool calls**, executes them in order, **evaluates** each step using heuristics, and only marks the goal complete after **rule-based verification** of evidence. It persists goals and step records to a local SQLite DB (`jarvis_state.db`) so you can inspect history across runs. The core execution loop is in `core/orchestrator.py`, while tools and tool outputs drive most of the system’s decision-making.
...
- Persistence via SQLite (`state/persistence.py`, DB: `jarvis_state.db`)
